"""
3D composition experiment for SITV.

This module implements the 3D task vector composition experiment that explores
loss landscapes under three-task composition.
"""
...
from sitv.data.models import ThreeDSweepResult
from sitv.core.evaluation import EvaluationService


class Composition3DExperiment(Experiment):
    """3D composition experiment: L(M_base + α·T1 + β·T2 + γ·T3).

    This experiment sweeps over a 3D grid of (α, β, γ) values to explore
    loss landscapes under three-task vector composition. It investigates
    whether patterns from 2D composition extend to higher dimensions.

    Attributes:
        task_vector_1: First task vector T1
        task_vector_2: Second task vector T2
        task_vector_3: Third task vector T3
        general_eval_texts: Evaluation texts
        alpha_range: Range of α values
        beta_range: Range of β values
        gamma_range: Range of γ values
    ...
    def __init__(
        ...
        num_samples_per_dim: int = 10,
        device: str = "cuda",
        eval_batch_size: int = 8,
        eval_enable_mixed_precision: bool = True,
        eval_max_length: int = 512,
    ):
        """Initialize the 3D composition experiment.
        ...
        Args:
            ...
            gamma_range: Range for γ (T3 scaling)
        ...
        self.task_vector_3 = task_vector_3
    ...
    def run(self) -> tuple[List[ThreeDSweepResult], Dict[str, Any]]:
        """Run the 3D composition experiment.

        Returns:
            Tuple of (results_list, metadata_dict)
                - results_list: List of ThreeDSweepResult objects
                - metadata_dict: Dictionary with timing and configuration info
        """
        self.start_timing()

        total_evals = self.num_samples_per_dim**3

        print(f"\n{'=' * 70}")
        print("3D TASK VECTOR COMPOSITION: L(M_base + α·T1 + β·T2 + γ·T3)")
        print(f"{'=' * 70}")
        print(f"α range: [{self.alpha_range[0]:.1f}, {self.alpha_range[1]:.1f}]")
        print(f"β range: [{self.beta_range[0]:.1f}, {self.beta_range[1]:.1f}]")
        print(f"γ range: [{self.gamma_range[0]:.1f}, {self.gamma_range[1]:.1f}]")
        print(
            f"Grid: {self.num_samples_per_dim}×{self.num_samples_per_dim}×{self.num_samples_per_dim} = {total_evals} evaluations"
        )
        print(f"Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print("\nQuestion: How do three tasks interact in the loss landscape?\n")

        # Prepare model
        self.prepare_model()

        # Clone original parameters (all task vectors must cover same params)
        original_params = self._clone_3d_parameters()

        # Pre-load task vectors to device for performance
        ...
        print("Pre-loading task vector 3 to device...")
        self.task_vector_3 = self.preload_task_vector_to_device(self.task_vector_3)
        ...
        gamma_values = np.linspace(
            self.gamma_range[0], self.gamma_range[1], self.num_samples_per_dim
        )
        ...
        for i, alpha in enumerate(alpha_values):
            for j, beta in enumerate(beta_values):
                for k, gamma in enumerate(gamma_values):
                    ...
                    print(
                        f"[{eval_count:5d}/{total_evals}] ({progress_pct:5.1f}%) "
                        f"α={alpha:+.2f}, β={beta:+.2f}, γ={gamma:+.2f} | ",
                        end="",
                        flush=True,
                    )

                    # Evaluate at (alpha, beta, gamma)
                    result = self._evaluate_at_alpha_beta_gamma(
                        alpha, beta, gamma, original_params, base_loss
                    )
    ...
    def _clone_3d_parameters(self) -> Dict[str, torch.Tensor]:
        """Clone parameters for 3D composition.
        ...
        for name, param in self.base_model.named_parameters():
            if (
                name in self.task_vector_1
                and name in self.task_vector_2
                and name in self.task_vector_3
            ):
                original_params[name] = param.clone().detach()
        return original_params

    def _evaluate_at_alpha_beta_gamma(
        self,
        alpha: float,
        beta: float,
        gamma: float,
        ...
        """Evaluate model at a specific (alpha, beta, gamma) point.

        Args:
            alpha: Alpha scaling factor for T1
            beta: Beta scaling factor for T2
            gamma: Gamma scaling factor for T3
            original_params: Original model parameters
            base_loss: Base model loss for reference

        Returns:
            ThreeDSweepResult with evaluation metrics
        """
        # Apply 3D composition: M(α,β,γ) = M_base + α·T1 + β·T2 + γ·T3
        self.apply_3d_composition(
            original_params,
            self.task_vector_1,
            self.task_vector_2,
            self.task_vector_3,
    ...
    def _print_summary(self, eval_times: List[float], total_evals: int) -> None:
        ...
        print(f"\n{'=' * 70}")
        print("3D COMPOSITION SWEEP COMPLETE")
        print(f"{'=' * 70}")
